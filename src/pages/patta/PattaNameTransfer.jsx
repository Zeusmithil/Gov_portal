...
export default function DrivingSchool() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      const dbData = await getSubService('patta-document', 'patta-name-transfer')
      setData(dbData)
    }
    loadData()
  }, [navigate])
  ...
  return (
    <>
      ...
      <div className="container" style={{ paddingBottom: '80px' }}>
        <button
          className="back-link"
          onClick={() => navigate('/process/patta-name-transfer')}
          style={{ marginBottom: '24px' }}
        >
          ← Back to Patta Document Services
        </button>

        <div className="page-header">
          <h1>Name Transfer</h1>
          <p>Apply for a name transfer patta</p>
        </div>
